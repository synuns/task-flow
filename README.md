...
React 19와 TypeScript로 구현한 업무 관리 SPA입니다. OpenAPI 기반 MSW mock API로
로그인부터 업무 탐색·수정·삭제까지 실행하고 검증할 수 있습니다.

| 항목 | 내용 |
| --- | --- |
| 주요 기능 | 인증, 대시보드, 회원정보, 가상화된 할 일 목록, User·Task CRUD |
| API | 원본 OpenAPI + 승인된 CRUD 확장 계약 + MSW |
| Architecture | FSD 기반 `app → pages → widgets → features → entities → shared` |
| Test | Vitest, Testing Library, Playwright, agent-browser |
...
User와 Task 변경 내용은 현재 탭의 `sessionStorage`에 저장됩니다. 초기 데이터로
되돌리려면 새 browser session을 열거나 해당 site의 session storage를 지운 뒤
reload합니다.
...
| 3 | [`docs/api/crud-openapi.yaml`](./docs/api/crud-openapi.yaml) | 승인 후 추가한 User·Task CRUD 계약 |
...
원본 범위를 구현한 뒤 별도 설계와 사람 승인을 거쳐 확장한 기능입니다.
...
browser evidence와 사람 checkpoint를 묶은 개발 단위입니다. route 수가 아니라 다음
경계를 기준으로 나눴습니다.
...
```mermaid
flowchart TD
    A[요구사항·OpenAPI 확인] --> B[Journey와 Requirement ID 선택]
    B --> C[격리 worktree 생성]
    C --> D[설계와 구현 계획 작성]
    D --> E[검증 가능한 작은 단위 구현]
    E --> F[Focused test]
    F -->|FAIL| G[실패 분류와 root cause 수정]
    G --> E
    F -->|PASS| H[pnpm verify quick]
    H --> I[Journey E2E와 browser QA]
    I --> J[TODO와 evidence 기록]
    J --> K[Plan-completion adversarial review]
    K --> L[Human checkpoint]
    L --> M[pnpm verify full과 final QA]
```
...
Journey 하나만 검증할 때는 대응하는 파일을 실행합니다.
...
| [구현 계획](./docs/superpowers/plans/) | 실행 순서와 검증 단계 |
| [AI 사용 내역](./AI_USAGE.md) | 도구, 모델, 스킬, 검증 범위 |
| [프롬프트 작업 기록](./artifacts/index.md) | 사람 검토를 마친 세션별 작업 주제 |
