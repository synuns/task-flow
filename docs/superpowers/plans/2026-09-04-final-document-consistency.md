...
- [ ] **Step 1: Markdown 링크와 로컬 경로를 검사한다**
...
- [ ] **Step 1: 각 finding의 authoritative source를 기록한다**
...
- [ ] **Step 1: canonical 검증을 실행한다**

Run: `pnpm verify quick`

Expected: hook/verifier, format, lint, OpenAPI type check, TypeScript와 전체 Vitest PASS.

- [ ] **Step 2: frozen target을 적대적으로 재검토한다**

중복, source priority, 현재/과거 시제, 링크, 경로, 명령, 상태, 수치, AI disclosure와 무관한 product diff를 다시 확인한다.

- [ ] **Step 3: evidence와 TODO를 완료 상태로 갱신한다**

`docs/quality/evidence/final-qa.md`에 Review target, Reviewer, Checks, Findings, Corrections, Rerun, Verdict를 기록하고 `DOCS-FINAL-CONSISTENCY-01`을 `AI_VERIFIED`로 전환한다.

- [ ] **Step 4: 최종 read-only gate를 실행한다**

Run: `pnpm verify setup && git diff --check`

Expected: setup PASS, whitespace error 0, 제품·OpenAPI·generated·dependency diff 0.
